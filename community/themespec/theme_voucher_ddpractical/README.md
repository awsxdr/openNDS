...
**Copy the themespec (theme_voucher_ddpractical.sh) file and the voucher.txt file.**

SSH into a terminal session on the router and use the following commands:

       cd /usr/lib/opennds
       wget https://raw.githubusercontent.com/openNDS/openNDS/master/community/themespec/theme_voucher_ddpractical/theme_voucher_ddpractical.sh
       cd /tmp/ndslog
       wget https://raw.githubusercontent.com/openNDS/openNDS/master/community/themespec/theme_voucher_ddpractical/vouchers.txt

**Now you need to activate the voucher script.**

Use the following commands:

       chmod 744 /usr/lib/opennds/theme_voucher_ddpractical.sh
       uci set opennds.@opennds[0].login_option_enabled='3'
       uci set opennds.@opennds[0].themespec_path='/usr/lib/opennds/theme_voucher_ddpractical.sh'
       uci commit opennds
...
Download it like this:
       ...
       wget https://raw.githubusercontent.com/openNDS/openNDS/master/community/themespec/theme_voucher_ddpractical/voucher_generator.py
...
It should be changed to an external storage medium. (See the last section of the `theme_voucher_ddpractical.sh` file, entitled "Customise the Logfile location").
